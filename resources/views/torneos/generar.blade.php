<x-panel-layout>
    <div class="mx-auto max-w-6xl">
        <div>
            <h1 class="text-3xl font-bold">Generar torneo</h1>
            <p class="mt-2 text-sm text-zinc-600">Crea un torneo vinculado a una temporada y genera sus partidos iniciales segun el formato elegido.</p>
        </div>
        ...
        <form method="POST" action="{{ route('gestion.torneos.generar.store') }}" class="mt-6 grid gap-6 lg:grid-cols-[1fr_.9fr]">
            @csrf

            <section class="rounded-lg border border-zinc-200 bg-white p-6">
                <h2 class="text-lg font-semibold">Reglas del torneo</h2>

                <div class="mt-5 grid gap-5 sm:grid-cols-2">
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-semibold text-zinc-700">Nombre</label>
                        <input name="nombre" value="{{ old('nombre') }}" class="mt-2 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm" placeholder="Copa de Verano">
                    </div>
                    ...
                    <div class="sm:col-span-2">
                        <label class="block text-sm font-semibold text-zinc-700">Tipo de torneo</label>
                        <select name="tipo" class="mt-2 w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm">
                            @foreach ($tiposTorneo as $valorTipo => $nombreTipo)
                                <option value="{{ $valorTipo }}" @selected(old('tipo') === $valorTipo)>{{ $nombreTipo }}</option>
                            @endforeach
                        </select>
                        <p class="mt-2 text-xs text-zinc-500">Mundial: grupos de 4 aprox. Champions: fase liga inicial. Personalizado: grupos opcionales o liga.</p>
                    </div>

                    <div>
                        <label class="block text-sm font-semibold text-zinc-700">Cantidad de grupos</label>
                        <input name="cantidad_grupos" type="number" min="1" max="16" value="{{ old('cantidad_grupos') }}" class="mt-2 w-full rounded-md border border-zinc-300 px-3 py-2 text-sm" placeholder="Opcional">
                    </div>
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-zinc-700">Tercer lugar</label>
                        <select name="tercer_lugar" class="mt-2 w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm">
            ...
            <section class="space-y-6">
                ...
                <div class="rounded-lg border border-zinc-200 bg-white p-6">
                    <h2 class="text-lg font-semibold">Arbitros</h2>
                    <div class="mt-4 max-h-48 space-y-2 overflow-auto">
                        ...
                        @empty
                            <p class="text-sm text-zinc-500">Sin arbitros registrados.</p>
                        @endforelse
                    </div>
                </div>
